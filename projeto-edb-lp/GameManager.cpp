...
int GameManager::Executar(ALLEGRO_EVENT_QUEUE * event_queue,  ALLEGRO_EVENT &ev, ALLEGRO_DISPLAY *display,
	...
	for(int j = 1; j <= 3; ++j)
	{
		...
		for (int i = 2; i <= 8; i+=2) {
			Horda* nova_horda = new Horda(idHorda, i, 2, 2*j, 10, 30, 60, proj_inimigo, 5);
		...
		switch(j){
			case 1:
				boss = new Horda(idHorda, 1, 3, 40*j, 10, 20, 15, proj_inimigo, 100);
				break;
			case 2:
				boss = new Horda(idHorda, 1, 3, 40*j, 10, 20, 15, proj_inimigo, 100);
				break;
			case 3: 
				boss = new Horda(idHorda, 1, 3, 40*j, 4, 20, 15, proj_inimigo, 100);
				break;
		}
